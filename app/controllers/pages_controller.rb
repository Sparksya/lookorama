class PagesController < ApplicationController
    ...
    def search
        if params[:search].present? && params[:search].strip != ""
           session[:lookorama_search] = params[:search]
        end
        
        arrResult = Array.new
        
        if session[:lookorama_search] && session[:lookorama_search] != ""
           @rooms_address = Room.where(active: true).near(session[:lookorama_search], 5, order:'distance')
